...
import static pl.coffeecode.coffeerepo.api.Predicate.greaterOrEqual;
...
public class ConditionGreaterOrEqualTest extends DBUnitTest {
	
	private static final String DATASET_FILE = "dbunit/condition_greater_or_equal.xml";

	@Test
	@Parameters(method = "databases")
	public void should_return_greater_or_equal_than_35(SQLDialectDatasource dialectDatasource) {
		prepare(dialectDatasource);
		QueryResult result = dsl
				
				.select(C_AGE)
				.from(VIEW_NAME)
				.where(greaterOrEqual(C_AGE, 35))
				.getResult();
	...
	public void should_ignore_case_when_comparing_strings(SQLDialectDatasource dialectDatasource) {
		...
		QueryResult result = dsl
				...
				.select(C_NAME)
				.from(VIEW_NAME)
				.where(greaterOrEqual(C_NAME, "kabana"))
				.getResult();
